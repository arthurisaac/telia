...
class PaymentScreenState extends State<PaymentScreen> {
  ...
  Future<void> getDateTime() async {
    try {
      ...
      Response response = await post(Uri.parse(getSettingsUrl),
              body: parameter, headers: ApiUtils.getHeaders())
    ...
    try {
      ...
      if (!error) {
        ...
        paytmResponse.then((value) {
          ...
          UiUtils.setSnackBar(
              StringsRes.payment, paymentResponse!, context, false);
        });
      } else {
        _placeOrder = true;
        UiUtils.setSnackBar(
            StringsRes.payment, getdata["message"], context, false);
      }
    } catch (e) {
      print(e);
    }
  ...
  razorpayPayment() async {
    ...
    if (contact != '' && email != '') {
      ...
      if (email == '') {
        ...
        UiUtils.setSnackBar(
            StringsRes.phoneNumber, 'phoneWarning', context, false);
      }
    }
  }
  ...
  payStackPayment(BuildContext context) async {
    ...
    try {
      ...
      if (response.status) {
        ...
        UiUtils.setSnackBar(
            StringsRes.payment, response.message, context, false);
        if (mounted) {
          setState(() {
            _placeOrder = true;
          });
  ...
  Future<void> placeOrder(String? tranId) async {
    ...
    try {
      ...
      Response response = await post(Uri.parse(placeOrderUrl),
              body: parameter, headers: ApiUtils.getHeaders())
          .timeout(const Duration(seconds: 50));
        ...
        if (!error) {
          ...
          if (paymentMethod == StringsRes.razorpayLbl) {
            ...
            addTransaction(stripePayId, orderId,
                tranId == "succeeded" ? placedKey : waitingKey, msg, true);
          } else if (paymentMethod == StringsRes.payStackLbl) {
            addTransaction(tranId, orderId, "Success", msg, true);
          } else if (paymentMethod == StringsRes.paytmLbl) {
            addTransaction(tranId, orderId, "Success", msg, true);
  ...
  Future<void> paypalPayment(String orderId) async {
    try {
      ...
      Response response = await post(Uri.parse(getPaypalLinkUrl),
              body: parameter, headers: ApiUtils.getHeaders())
          .timeout(const Duration(seconds: 50));
  ...
  stripePayment() async {
    var response = await StripeService.payWithPaymentSheet(
        amount: (finalTotal.toInt() * 100).toString(),
  ...
  Future<void> addTransaction(String? tranId, String orderID, String? status,
    ...
    try {
      ...
      Response response = await post(Uri.parse(addTransactionUrl),
              body: parameter, headers: ApiUtils.getHeaders())
          .timeout(const Duration(seconds: 50));
  ...
  Future<void> flutterWavePayment() async {
    try {
      ...
      Response response = await post(Uri.parse(flutterwaveWebviewUrl),
              body: parameter, headers: ApiUtils.getHeaders())
          .timeout(const Duration(seconds: 50));
